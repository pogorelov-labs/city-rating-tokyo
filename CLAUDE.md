...
| `app/src/components/LiveCameras.tsx` | Click-to-load facade → iframe on user click. 16:9 aspect, real YouTube thumbnail preview on facade (with dark gradient overlay for readability), tab strip when 2+ cameras, ✕ dismiss overlay, `role="tabpanel"` + `aria-labelledby`, `youtube.com` embed with `autoplay=1&mute=1&playsinline=1`. Requires CSP `frame-src` + `img-src i.ytimg.com` in `next.config.ts`. `next/dynamic` not needed — already `'use client'` and conditionally rendered (only stations with livecams mount it). |
